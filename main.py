...
if __name__ == '__main__':
    ...
    for i in tqdm(range(n_games)):
        ...
        while not done:
            ...
            n_steps += 1
            score += reward
            agent.remember(observation.cpu().detach().numpy(), action, prob, val, reward, done)
            if n_steps % N == 0:
                agent.learn()
                learn_iters += 1
    ...
    with open("episode_rewards_3000N.txt", "wb") as fp:
        pickle.dump(score_history, fp)
    with open("moving_average_3000N.txt", "wb") as fp:
